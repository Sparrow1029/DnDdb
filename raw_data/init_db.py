...
from class_dicts import RESTRICTIONS_DICT, SAVING_THROWS_DICT, TO_HIT_DICT, SUMMARIES
import sys
...
class Database:
        ...
        for c in classes:
            ...
            for row in csv_lvl_adv:
                ...
                # Rangers can use Druid and/or Mage spells after level 8-9
                    ...
                    level_adv_embedded_document["spellcasting_level"] = int(
                        row["ranger_spell_casting_lvl"]
                    )
                    level_adv_embedded_document["spells_by_level"] = {
                        "druid_spells": {
                ...
                # Paladins can cast Cleric spells after 9th level
                    ...
                    level_adv_embedded_document["spellcasting_level"] = row[
                        "paladin_cleric_sp_lvl"
                    ]
                    level_adv_embedded_document["spells_by_level"] = {
    ...
    def init_db_spells(self, spell_csv="all_spells.csv", coll="spell_collection"):
        ...
        for row in reader:
            ...
            if not row["embedded_tables"]:
                ...
                row["embedded_tables"] = self.parse_embedded_tables(
                    row["embedded_tables"]
                )
    ...
    def init_db_races(self, race_csv_file="race_data.csv", coll="race_collection"):
        ...
        for row in reader:
            row["languages"] = list(
                map(lambda s: s.replace("_", " "), row["languages"].split())
            )
    ...
    def init_db_inventory(
        self, items_csv="items.csv", armor_csv="armor.csv", weapons_csv="weapons.csv"
    ):
    ...
    def init_db_weapons(self, weapons_csv="weapons.csv"):
        ...
        for row in weapons_reader:
            document = {
                ...
                "type": row["type"],
                "rate_of_fire": float(row["rate_of_fire"])
                if row["rate_of_fire"]
                else None,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--delete-all", action="store_true", help="delete all collections, and exit"
    )
    parser.add_argument(
        "--create-all", action="store_true", help="create all collections, and exit"
    )
    parser.add_argument(
        "-d",
        nargs="*",
        metavar="collection",
        help="delete specified collections (items, armor, weapons, spell, race, classes)",
        default=[],
    )
    parser.add_argument(
        "-c",
        nargs="*",
        metavar="collection",
        help="(create specified collections: classes, spells, races, inventory)",
        default=[],
    )
